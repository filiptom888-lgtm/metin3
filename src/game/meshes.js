...
export function createScene() {
  ...
  const colors = ["#c8b090", "#b8a078", "#d0b898", "#a89070", "#c0a888"];
  const layouts = [
    [11, -8], [-12, -7],
  ...
  layouts.forEach(([bx, bz], i) => {
    ...
    const b = makeBuilding(
      ...
      { roofColor: ["#8a3a30", "#6a3a48", "#7a4a28", "#5a2a28"][i % 4] }
    );
...
export function makeValleyMapRoot() {
  ...
  const adobe = ["#d2b48c", "#c4a574", "#e0c4a0", "#b8956a", "#cbb08a"];
  const layouts = [
...
export function makeOrcMapRoot() {
  ...
  // Thick dark forest around the tower
  ...
  for (let i = 0; i < 90; i++) {
    ...
    // Keep the teleporter pad clear
    if (Math.hypot(x, z - 14) < 5) continue;
    const g = new THREE.Group();
    const h = 3.2 + Math.random() * 4.5;
    const t = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.38, h, 6), trunk);
    t.position.y = h / 2;
    t.castShadow = true;
    g.add(t);
    const c1 = new THREE.Mesh(
      new THREE.ConeGeometry(1.4 + Math.random() * 1.2, 2.8 + Math.random() * 2, 6),
    ...
    if (Math.random() < 0.55) {
      const c2 = new THREE.Mesh(
        new THREE.ConeGeometry(1.0 + Math.random() * 0.8, 2.2, 6),
        canopyDark
      );
      c2.position.y = h + 1.6;
      c2.castShadow = true;
  ...
  // Sparse trees on outer islets
  for (const isle of ORC_ISLANDS) {
    if (isle.tier === "main") continue;
    const n = 4 + ((Math.random() * 5) | 0);
    for (let i = 0; i < n; i++) {
      const ang = Math.random() * Math.PI * 2;
      const r = 2 + Math.random() * (isle.r - 3.5);
      const g = new THREE.Group();
      const h = 2.2 + Math.random() * 2.4;
      const t = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.28, h, 5), trunk);
      t.position.y = h / 2;
      t.castShadow = true;
      g.add(t);
      const c1 = new THREE.Mesh(new THREE.ConeGeometry(1.1, 2.2, 5), canopyDark);
      c1.position.y = h + 0.2;
      c1.castShadow = true;
      g.add(c1);
...
export function animateCharacter(
  ...
  if (attackPulse) {
    ...
    // Local: mesh mirrors player countdown (already decremented in Game.update)
    d.attacking = attacking;
    if (attackDur > 0) d.attackDur = attackDur;
  } else {
    // Remote: drain after rising-edge pulse
